...
class AESCipher(object):
    ...
    def encrypt(self, raw):
        ...
        ciphertext, tag = cipher.encrypt_and_digest(Padding.pad(raw.encode(), AES.block_size))
        # безопасно, так как GCM (Galois/Counter Mode) использует IV, tag (целостность) и зашифрованный текст
    ...
    def decrypt(self, enc):
        ...
        decrypted_data = Padding.unpad(cipher.decrypt_and_verify(ciphertext, tag), AES.block_size)
...
class DataAnonymizer:
    ...
    def anonymize(self, data):
        ...
        uuid_ = uuid.uuid4()
        new_row = pd.DataFrame([[uuid_, source_encrypted, new_data]], columns=['uuid', 'source_encrypted', 'data'])
...
def anonymize_date(dt: str) -> str:
    ...
    start_date = datetime.datetime(parts[2], abs(parts[1] - 1 % 12), abs(parts[0] - 28 % 12))
    end_date = datetime.datetime(parts[2], abs(parts[1] + 1 % 12), abs(parts[0] - 28 % 12))
